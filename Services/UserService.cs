...
public class UserService(UserManager<ApplicationUser> userManager,IRoleService roleService, ApplicationDbContext context) : IUserService
{
    ...
    private readonly IRoleService _roleServie= roleService;
    ...
    public async Task<Result> UpdateProfileAsync(string userId, UserUpdateRequest request)
    {
        await _userManager
           .Users
           .Where(x => x.Id == userId)
    ...
    public async Task<IEnumerable<UserResponse>> GetAllAsync(CancellationToken cancellationToken = default) =>
        ...
        .Select(x => new UserResponse(x.Key.Id, x.Key.FirstName, x.Key.LastName, x.Key.Email, x.Key.IsDisabled, x.Select(x => x.Name))).ToListAsync();
    ...
    public async Task<Result<UserResponse>> GetUserAsync(string userId, CancellationToken cancellationToken = default)
    {
        ...
        var response = new UserResponse(userId, user.FirstName, user.Email!, user.LastName, user.IsDisabled, userRoles);

        return Result.Success<UserResponse>(response);

    }

    public async Task<Result<UserResponse>> AddAsync(CreateUsreRequest request, CancellationToken cancellationToken = default)
    {
        var emailIsExists = _userManager.Users.Any(x => x.Email == request.Email);
        if (emailIsExists) return Result.Failure<UserResponse>(UserErrors.DuplicatedEmail);

        var user = request.Adapt<ApplicationUser>();
       var allowRoles=await _roleServie.GetAllRolesAsync(false, cancellationToken);
        ...
        if(result.Succeeded)
        {
            ...
            var response = new UserResponse(user.Id, user.FirstName, user.Email!, user.LastName, user.IsDisabled, request.Roles);
    ...
    public async Task<Result> UpdateAsync(string Id,UpdateUserRequest request, CancellationToken cancellationToken = default)
    {
        ...
        if (emailIsExists) return Result.Failure(UserErrors.DuplicatedEmail);
        ...
        if (result.Succeeded)
        {
            await _context.UserRoles.Where(x=>x.UserId==Id)
                .ExecuteDeleteAsync();
            await _userManager.AddToRolesAsync(user, request.Roles);
